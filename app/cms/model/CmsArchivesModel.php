...
class CmsArchivesModel extends Model
{
    ...
    public function adminAddArticle($data, $categories)
    {
		...
		if(!empty($data['content'])) {
			//获取简介
			$description = html2text($data['content']);
			$description = diy_msubstr($description,0, '120', 'utf-8', false);
    ...
    public function addTags($keywords, $articleId)
    {
        ...
        if (!empty($keywords)) {
            ...
            foreach ($keywords as $keyword) {
                ...
                if (!empty($keyword)) {
                    //判断是已存在
                    $findTag = $cmsTagsModel->where('name', $keyword)->where('type','1')->find();
                    if (empty($findTag)) {
                        //如果搜索不存在 那么进行插入
                        ...
                        //如果存在此关键词进行插入（需要匹配是否已含有）
                        $oldTagIdsa = $cmsTagsModel->where('name', $keyword)->where('type','1')->column('archives');
                        $id_newa = $cmsTagsModel->where('name', $keyword)->where('type','1')->column('id');
                                  $oldTagIds=explode(',',$oldTagIdsa[0]);
                                  $oldTagIds=array_filter($oldTagIds);//去除空值 0 false null
                                 ...
                                 if (!in_array($articleId, $oldTagIds)) {
                                    ...
                                    $result = implode(',', $oldTagIds);
                                     $cmsTagsModel->where('name', $keyword)->where('type','1')->update(['archives' => $result,'nums'=>$archives_num]);
            ...
            //一些关键词删除 后 tag表中需要同步删除  tag_ids 数组为 该文章最新的关键词 id数组

               $arrs = $cmsTagsModel->where('archives','like','%'.$articleId."%")->where('type','1')->select()->toArray();//含有当前文章的 tag IDS 
               ...
               foreach ($arrs as $k => $v) {
                if(!in_array($v['id'],$tag_ids)){
                   ...
                   if(!empty($v['archives'])){
                         ...
                         $len_my=array_filter($len_my);//去除空值 0 false null
                      ...
                      foreach ($len_my as $k1 => $v1) {
                       ...
                       //新增
                        $oldnum=count($len_my);
                        $cmsTagsModel->where('id', $v['id'])->update(['archives' => implode(',',$len_my),'nums'=>$oldnum]); 
