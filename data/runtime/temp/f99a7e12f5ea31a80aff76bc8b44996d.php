...
<?php if(is_array($lists) || $lists instanceof \think\Collection || $lists instanceof \think\Paginator): if( count($lists)==0 ) : echo "" ;else: foreach($lists as $k=>$vo): 
   ...
   ?>
    <dl> 
    <dt>
    <a href="<?php echo cmf_url('cms/Archives/index',array('id'=>$vo['id'],'cid'=>$vo['channel_id'])); ?>" target="_self" class="scale-img">
      <?php if($vo['more'] != ''): ?>
    ...
    <dd> 
     <h3 class="ellipsis">
      <a href="<?php echo cmf_url('cms/Archives/index',array('id'=>$vo['id'],'cid'=>$vo['channel_id'])); ?>" target="_self" title="">
        <?php echo str_replace($keyword,"<font style='color:#d5417b'>".$keyword."</font>",$vo['title']) ?></a>
      </h3> 
      <div class="tit_addon">
        <span class="time"><?php echo date('Y-m-d H:i:s',$vo['update_time']); ?></span>
        <?php 
         ...
         ?>
        <span class="tags">
            <?php if(is_array($keywords_arr) || $keywords_arr instanceof \think\Collection || $keywords_arr instanceof \think\Paginator): if( count($keywords_arr)==0 ) : echo "" ;else: foreach($keywords_arr as $key=>$vk): if($vk != ''): 
                      $taginfo= Db::name('CmsTags')->where("type = 1")->where('name','=',$vk)->order('nums desc')->find();
                   ?>
                  <a href="<?php echo cmf_url('cms/List/indextag',array('id'=>$taginfo['id'])); ?>" target="_blank"><?php echo $vk; ?></a>
              <?php endif; ?>
            <?php endforeach; endif; else: echo "" ;endif; ?>
        </span>
      </div> 
      <p>
        <a href="<?php echo cmf_url('cms/Archives/index',array('id'=>$vo['id'],'cid'=>$vo['channel_id'])); ?>" target="_self" title="">     
          <?php $jq=mb_substr($vo['description'],0,200,'utf-8');echo str_replace($keyword,"<font style='color:#d5417b'>".$keyword."</font>",$jq) ?>...
        </a>
      </p> 
    </dd>
    <a class="plus">+</a>
 ...
 <div class="NoSearchResults">
      <div class="bt_bb_content"> 对不起，没有匹配到相关信息: <span class="R_search_keyword">关键词</span></div>
        <form role="search" method="get" id="searchform" class="searchform" action="/cms/search/index" method="post" >
                    <input type="search" name="keyword" value="<?php echo $keyword; ?>" placeholder="Looking for..."  id="keyword">
