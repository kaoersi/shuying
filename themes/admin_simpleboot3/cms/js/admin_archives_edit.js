                    ...
                    for (i = 0; i < array.length; i++) {
                        ...
                        switch (item.type.toLowerCase()) {
                            ...
                            case "image"://单图片
                                ...
                                html += '        <a href="javascript:uploadOneImage(\'图片上传\',\'#' + item.name + '\');" class="upload_img">';
                                html += '            <img src="' + item.value + '" id="' + item.name + '-preview" width="60" height="60" style="cursor: pointer" onerror="javascript:this.src=\'/themes/admin_simpleboot3/public/assets/images/default-thumbnail.png\';" />';
                                html += '        </a>';
                            ...
                            case "manyinfo"://多信息
                                ...
                                html += '    <td>';
                                html += '        <ul id="' + item.name + '" class="pic-list list-unstyled form-inline">' + item.value + '</ul>';
                                html += '        <a href="javascript:addManyinfo(\'#' + item.name + '\',\'' + item.name + '-item-tpl\');" class="btn btn-default btn-sm">添加一组</a>  ';
                                html += '        <input type="hidden" name="post[manyinfo][]" value="' + item.name + '">';//为了插入处理，而增加的
                                html += '    </td>';
                                html += '</tr>';

                                uploadtxt += '<script type="text/html" id="' + item.name + '-item-tpl">';
                                uploadtxt += '    <li id="saved-manyinfo{id}">';
                                uploadtxt += '        <input id="manyinfo-{id}" type="hidden" name="post[' + item.name + '_urls][]" value="{filepath}">';
                                uploadtxt += '        <input class="form-control" type="text" name="post[' + item.name + '_titles][]" style="width: 250px;" placeholder="文字1">';
                                uploadtxt += '        <input class="form-control" type="text" name="post[' + item.name + '_descs][]" style="width: 250px;" placeholder="文字2">';
                                uploadtxt += '        <img id="manyinfo-{id}-preview" src="{url}" style="height:36px;width: 36px;" onerror="javascript:this.src=\'/themes/admin_simpleboot3/public/assets/images/default-thumbnail.png\';" onclick="imagePreviewDialog(this.src);">';
                                uploadtxt += '        <a href="javascript:uploadOneImage(\'图片上传\',\'#manyinfo-{id}\');">添加图片</a>';
                                uploadtxt += '        <a href="javascript:(function(){$(\'#saved-manyinfo{id}\').remove();})();">移除</a>';
                                uploadtxt += '    </li>';
                                uploadtxt += '<\/script>';
						 
                                break;		

                            case "manydesc"://多带图带简介多信息
                                html += '<tr>';
                                html += '    <th>' + item.title + '</th>';
                                html += '    <td>';
                                html += '        <ul id="' + item.name + '" class="pic-list list-unstyled form-inline">' + item.value + '</ul>';
                                html += '        <a href="javascript:addManyinfo(\'#' + item.name + '\',\'' + item.name + '-item-tpl\');" class="btn btn-default btn-sm">添加一组</a>  ';
                                html += '        <input type="hidden" name="post[manydesc][]" value="' + item.name + '">';//为了插入处理，而增加的
                                html += '    </td>';
                                html += '</tr>';

                                uploadtxt += '<script type="text/html" id="' + item.name + '-item-tpl">';
                                uploadtxt += '    <li id="saved-manydesc{id}">';
                                uploadtxt += '        <p"><input id="manydesc-{id}" type="hidden" name="post[' + item.name + '_urls][]" value="{filepath}">';
                                uploadtxt += '        <input class="form-control" type="text" name="post[' + item.name + '_titles][]" style="width: 250px;" placeholder="文字1">';
                                uploadtxt += '        <input class="form-control" type="text" name="post[' + item.name + '_stits][]" style="width: 250px;" placeholder="文字2">';
                                 uploadtxt += '        <img id="manydesc-{id}-preview" src="{url}" style="height:36px;width: 36px;" onerror="javascript:this.src=\'/themes/admin_simpleboot3/public/assets/images/default-thumbnail.png\';" onclick="imagePreviewDialog(this.src);">';

                                uploadtxt += '        <a href="javascript:uploadOneImage(\'图片上传\',\'#manydesc-{id}\');">添加图片</a>';
                                uploadtxt += '        <a href="javascript:(function(){$(\'#saved-manydesc{id}\').remove();})();">移除</a></p>';

                                uploadtxt += '        <p><textarea style="width:504px" placeholder="文字简介3" class="form-control" rows="3" name="post[' + item.name + '_descs][]"></textarea></p>';                        
                            ...
                            case "manyinput"://多信息
                                ...
                                html += '        <ul id="' + item.name + '" class="pic-list list-unstyled form-inline">' + item.value + '</ul>';
                                html += '        <a href="javascript:addManyinfo(\'#' + item.name + '\',\'' + item.name + '-item-tpl\');" class="btn btn-default btn-sm">添加一组</a>  ';
                                html += '        <input type="hidden" name="post[manyinput][]" value="' + item.name + '">';//为了插入处理，而增加的
                                html += '    </td>';
                                html += '</tr>';

                                uploadtxt += '<script type="text/html" id="' + item.name + '-item-tpl">';
                                uploadtxt += '    <li id="saved-manyinput{id}">';
                               
                                uploadtxt += '        <input class="form-control" type="text" name="post[' + item.name + '_titles][]" style="width: 250px;" placeholder="文字1">';
                                uploadtxt += '        <input class="form-control" type="text" name="post[' + item.name + '_descs][]" style="width: 250px;" placeholder="文字2">';
                            
                                uploadtxt += '        <a href="javascript:(function(){$(\'#saved-manyinput{id}\').remove();})();">移除</a>';
                                uploadtxt += '    </li>';
                                uploadtxt += '<\/script>';
